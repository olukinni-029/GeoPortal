...
function legend() {
	...
	overlays
		...
		.forEach((layer) => {
			...
			var img = new Image();
			img.src =
				'http://24.199.122.17:8080/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=' +
				layer.get('title');
...
$(document).ready(function () {
	$.ajax({
		type: 'GET',
		url: 'http://24.199.122.17:8080/geoserver/web/?1/wfs?request=getCapabilities',
		dataType: 'xml',
	...
	$('#layer').change(function () {
		...
		$(document).ready(function () {
			$.ajax({
				type: 'GET',
				url:
					'http://24.199.122.17:8080/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=' +
					value_layer,
				dataType: 'xml',
				success: function (xml) {
...
$(document).ready(function () {
	$.ajax({
		type: 'GET',
		url: 'http://24.199.122.17:8080/geoserver/wfs?request=getCapabilities',
		dataType: 'xml',
		success: function (xml) {
			var select = $('#layer1');
			$(xml)
...
function query() {
	...
	var url =
		'http://24.199.122.17:8080/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=' +
		value_layer +
		'&CQL_FILTER=' +
		value_attribute +
		'%20' +
...
function wms_layers() {
	...
	$(document).ready(function () {
		$.ajax({
			type: 'GET',
			url: 'http://24.199.122.17:8080/geoserver/wms?request=getCapabilities',
			dataType: 'xml',
			success: function (xml) {
...
function add_layer() {
	...
	var url = 'https://24.199.122.17:8080/geoserver/wms?request=getCapabilities';
		...
		.forEach((layer) => {
			...
			if (visibility == true) {
				...
				var wmsSource = new ol.source.ImageWMS({
					url: 'https://24.199.122.17:8080/geoserver/wms',
					params: {
						LAYERS: layer_title,
					},
					serverType: 'geoserver',
	...
	if (value !== 'None') {
		...
		if (draw_type.value == 'select' || draw_type.value == 'clear') {
			...
			draw1.on('drawend', function (evt) {
				...
				var url =
					'http://24.199.122.17:8080/geoserverwfs?request=GetFeature&version=1.0.0&typeName=' +
					value_layer +
		...
		datasets: [
			{
				...
				data: [],
				borderWidth: 0,
				backgroundColor: ['Red', 'Brown', 'Yellow', '#90EE90', 'Green'], //update colors for category
			},
		],
	},
	options: {
...
var sentinelLayer = new ol.layer.Image({
	title: 'sentinel_layer',

	source: new ol.source.ImageWMS({
		url: 'http://24.199.122.17:8080/geoserver/wms',
		params: {
			LAYERS: 'NDVI:sentinel2processed',
			VERSION: '1.1.0',
		},
		ratio: 1,
		serverType: 'geoserver',
...
const NDVIID = 'ndvi-data';
const SentinelID = 'sentinel-data';

NDVIDropDownIsClicked = false;
SentinelDropDownIsClicked = false;
//Display layers on the map
document.getElementById(NDVIID).addEventListener('click', () => {
	NDVIDropDownIsClicked = !NDVIDropDownIsClicked;
	if (NDVIDropDownIsClicked) {
		map.addLayer(ndviLayer);
	} else {
		map.removeLayer(ndviLayer);
	}
});
document.getElementById(SentinelID).addEventListener('click', () => {
	SentinelDropDownIsClicked = !SentinelDropDownIsClicked;
	if (SentinelDropDownIsClicked) {
...
//Listens to click events. When you click somewhere within the layers
// this function will grab the coordinates and make a request to the Geoserver to get the
	...
	fetch(ndviURL)
		...
		.then((data) => {
			if (data.numberReturned === 1) {
				chartValues = data.features[0].properties;

				const sumOfNDVI = Object.values(chartValues).reduce(
					(partialSum, a) => partialSum + a,
					0
				);

				//category 1
				const categoryOne = Object.values(chartValues)
					.filter((value) => value >= -1 && value <= 0.2)
					.reduce((partialSum, a) => partialSum + a, 0);

				categoryOnePercentage = (categoryOne / sumOfNDVI) * 100;

				//category 2
				const categoryTwo = Object.values(chartValues)
					.filter((value) => value >= 0.2 && value <= 0.4)
					.reduce((partialSum, a) => partialSum + a, 0);

				categoryTwoPercentage = (categoryTwo / sumOfNDVI) * 100;
				//category 3
				const categoryThree = Object.values(chartValues)
					.filter((value) => value >= 0.4 && value <= 0.6)
					.reduce((partialSum, a) => partialSum + a, 0);

				categoryThreePercentage = (categoryThree / sumOfNDVI) * 100;
				//category 4
				const categoryFour = Object.values(chartValues)
					.filter((value) => value >= 0.6 && value <= 0.8)
					.reduce((partialSum, a) => partialSum + a, 0);

				categoryFourPercentage = (categoryFour / sumOfNDVI) * 100;

				//category 5
				const categoryFive = Object.values(chartValues)
					.filter((value) => value >= 0.8 && value <= 1.0)
					.reduce((partialSum, a) => partialSum + a, 0);

				categoryFivePercentage = (categoryFive / sumOfNDVI) * 100;

				filteredValues = Object.values(chartValues).filter(
					(value) => value > 0.2
				);
				...
				pieChart.data.datasets.forEach((dataset) => {
					dataset.data.push(
						...[
							...
							categoryThreePercentage,
							categoryFourPercentage,
							categoryFivePercentage,
						]
					);
				});

				//update the chart labels
				chart.data.labels.push(...Object.keys(chartValues));

				//update the chart data
				chart.data.datasets.forEach((dataset) => {
					dataset.data.push(...filteredValues);
				});
